...
class AddGalaxyForm(FlaskForm):
    ...
    classification = SelectMultipleField(u'Classification (Press Shift to Select All That Apply)', choices = [('LBG (Lyman Break Galaxy)', 'LBG (Lyman Break Galaxy)'), ('MS (Main Sequence Galaxy)', 'MS (Main Sequence Galaxy)'), ('SMB (Submillimeter Galaxy)', 'SMB (Submillimeter Galaxy)'), ('DSFG (Dusty Star-Forming Galaxy)', 'DSFG (Dusty Star-Forming Galaxy)'), ('SB (Starburst)', 'SB (Starburst)'), ('AGN (Contains a Known Active Galactic Nucleus)', 'AGN (Contains a Known Active Galactic Nucleus)'), ('QSD (Optically Bright AGN)', 'QSD (Optically Bright AGN)'), ('Quasar (Optical and Radio Bright AGN)', 'Quasar (Optical and Radio Bright AGN)'), ('RQ-AGN (Radio-Quiet AGN)', 'RQ-AGN (Radio-Quiet AGN)'), ('RL-AGN (Radio-Loud AGN)', 'RL-AGN (Radio-Loud AGN)'), ('RG (Radio Galaxy)', 'RG (Radio Galaxy)'), ('BZK (BZK-Selected Galaxy)', 'BZK (BZK-Selected Galaxy)')], validators = [DataRequired ()])
...
class AddLineForm(FlaskForm):
    ...
    observed_beam_major = FloatField('Observed Beam Major (strongly recommended) ', validators = [Optional (), NumberRange(min = 0)])
    observed_beam_minor = FloatField('Observed Beam Minor (strongly recommended) ', validators = [Optional (), NumberRange(min = 0)])
...
class EditLineForm(FlaskForm):
    ...
    freq_type = StringField('Enter as redshift (z) or observed frequency (f)', validators = [DataRequired ()])
    ...
    detection_type = StringField('Detection Type - Single Dish or Interferometric', validators = [Optional ()])
    observed_beam_major = FloatField('Observed Beam Major (strongly recommended) ', validators = [Optional (), NumberRange(min = 0)])
    observed_beam_minor = FloatField('Observed Beam Minor (strongly recommended) ', validators = [Optional (), NumberRange(min = 0)])
    observed_beam_angle = FloatField('Observed Beam Position Angle in Degrees (strongly recommended) ', validators = [Optional ()])
    reference = StringField('Citation (use ADS bibcode, example:  2019ApJ...879...52S)', validators = [DataRequired()])
