...
def main():
    ...
    lines = session.query(Line.galaxy_id, Line.emitted_frequency).distinct().all()
        ...
        for row in data:
            ...
            if g_validated == True:
                ...
                if row[COL_NAMES['emitted_frequency']] == "":
                    l_validated = False
                    flash ("Emitted Frequency is Mandatory")
                ...
                if row[COL_NAMES['species']] == "":
                    l_validated = False
                    flash ("Please Specify the Species")
