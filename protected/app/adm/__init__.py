    ...
    def post_view(self):
                ...
                if key == 'delete':
                    for id in list_of_ids:
                        ...
                        if templine_id != None:
                            ...
                            return "one of the posts has an id ({}) compatability issue, it doesn't seem to belong to any group".format(
                                id)
                        db.session.delete(post)
                        db.session.commit()
                elif key == 'approve':
        ...
        for id in list_of_tempgalaxy_ids:
            ...
            if similar_galaxy != None:
                ...
                similar_lines_approved_count = session.query(func.count(Line.id)).filter(
...
def post_approve(id):
    ...
    if templine_id != None:
        ...
        galaxy = session.query(TempGalaxy.name, TempGalaxy.right_ascension, TempGalaxy.declination,
        ...
        galaxy = session.query(EditGalaxy.name, EditGalaxy.right_ascension, EditGalaxy.declination,
                               EditGalaxy.coordinate_system, EditGalaxy.lensing_flag, EditGalaxy.classification,
                               EditGalaxy.notes, EditGalaxy.original_id).filter(EditGalaxy.id == editgalaxy_id).first()
        ...
        editline = EditLine.query.filter_by(id=editline_id).first()
        line = session.query(EditLine.galaxy_id, EditLine.original_line_id, EditLine.emitted_frequency,
                             ...
                             EditLine.peak_line_flux_uncertainty_positive, EditLine.peak_line_flux_uncertainty_negative,
                             ...
                             EditLine.observed_beam_major, EditLine.observed_beam_minor, EditLine.observed_beam_angle,
