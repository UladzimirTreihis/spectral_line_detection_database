...
class AdvancedSearchForm(FlaskForm):
    ...
    right_ascension_min = StringField('Right Ascension min:', validators=[
        Regexp(ra_reg_exp, message="Input in the format 00h00m00s or as a float [-90, +90]"), Optional()])
    right_ascension_max = StringField('Right Ascension max:', validators=[
        Regexp(ra_reg_exp, message="Input in the format 00h00m00s or as a float [0, 360]"), Optional()])
    ...
    declination_point = StringField('Declination', validators=[
        Regexp(dec_reg_exp, message="Input in the format (+/-)00d00m00s or as a float [0, 360]"), Optional()])
...
class AddGalaxyForm(FlaskForm):
    ...
    right_ascension = StringField('Right Ascension',
                                  validators=[Regexp(ra_reg_exp, message="Input in the format 00h00m00s or as a float [-90, +90]"),
                                              DataRequired()])
    declination = StringField('Declination', validators=[
        Regexp(dec_reg_exp, message="Input in the format (+/-)00d00m00s or as a float [0, 360]"), DataRequired()])
...
class AddLineForm(FlaskForm):
    ...
    observed_beam_angle = FloatField('Beam Position Angle (in degrees) (strongly recommended)',
                                     validators=[Optional()])
    reference = StringField('Citation (use ADS bibcode, example:  2019ApJ...879...52S)', validators=[Regexp(ref_reg_exp, message="Check Bibcode format"), DataRequired()])
    notes = StringField('Notes', validators=[Optional()])
    submit = SubmitField('Submit')
